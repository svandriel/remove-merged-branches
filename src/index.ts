...
export async function removeMergedBranches(
    ...
    log.verbose(`Branch ${chalk.cyan(fromBranch)} resolves to hash ${chalk.green(trimmedGitHash)}`);

    const { stdout: branches } = await runGit('branch', '--merged', trimmedGitHash);
    ...
    if (offline) {
        ...
        const { stdout: remoteBranches } = await runGit('branch', '--remote', '--merged', trimmedGitHash);

        const allRemoteBranches = parseBranchLines(remoteBranches);
        const remoteBranchLines = removeBranchFromList(fromBranch, allRemoteBranches);
        ...
        if (keys(branchesGroupedByRemote).length === 0) {
            ...
            const removalFn = dryRun ? removeRemoteBranchesDryRun : removeRemoteBranches;
...
async function removeRemoteBranches([remote, branchNames]: [string, string[]]): Promise<void> {
    log.info(`Removing from remote ${chalk.green(remote)}: ${chalk.cyan(branchNames.join(', '))}`);
    await runGit('push', '--no-verify', '--delete', remote, ...branchNames);
}

async function removeRemoteBranchesDryRun([remote, branchNames]: [string, string[]]): Promise<void> {
    log.info(`Would remove from remote ${chalk.green(remote)}: ${chalk.cyan(branchNames.join(', '))}`);
}
