...
export function runProcess(cmd: string, args: string[]): Promise<RunProcessOutput> {
    args = args || [];
            ...
            proc.on('exit', code => {
                ...
                if (code === 0) {
                    ...
                    const err = new RunProcessError(
                        `Process terminated with code ${code}: ${cmd} ${args.join(' ')}`,
                        output
                    );
                    reject(err);
                }
...
function logData(isError: boolean, data: any): void {
    const prefix = isError ? '!' : '>';
    const color = isError ? 'red' : 'grey';
    trimEnd(data.toString())
        .split('\n')
