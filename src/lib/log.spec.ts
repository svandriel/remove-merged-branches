...
test('logging on verbose level', () => {
    ...
    expect(console.log).not.toHaveBeenCalledWith(chalk.gray('VERBOSE'), 'Verbose2');

    log.setLevel(LogLevel.WARN);
    log.verbose('Verbose3');
    expect(console.log).not.toHaveBeenCalledWith(chalk.gray('VERBOSE'), 'Verbose3');
});
...
test('logging on warn level', () => {
    ...
    log.warn('Warning3');
    expect(console.log).not.toHaveBeenCalledWith(chalk.yellow('WARN'), 'Warning3');
});
